<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white font-heading">{{ __('Suppliers') }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Manage your supplier network') }}</p>
        </div>
        <a href="{{ route('filament.admin.resources.suppliers.create') }}"
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($suppliers as $supplier)
                        <tr wire:key="supplier-{{ $supplier->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{{ $supplier->name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $supplier->contact_person ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $supplier->email ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $supplier->products_count ?? 0 }}</td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('filament.admin.resources.suppliers.edit', $supplier) }}" class="text-amber-600 hover:text-amber-700">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-6 py-12 text-center text-gray-500">{{ __('No suppliers found') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($suppliers->hasPages())
            <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700">{{ $suppliers->links() }}</div>
        @endif
    </div>
</div>
