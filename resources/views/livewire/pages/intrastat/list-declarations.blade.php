<div>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white font-heading">{{ __('Intrastat Declarations') }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('EU intra-community trade declarations') }}</p>
        </div>
        ...
        <div class="p-4 flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="{{ __('Search declarations...') }}"
            ...
            <div class="sm:w-40">
                <select wire:model.live="direction" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white">
                    <option value="">{{ __('All directions') }}</option>
                    @foreach($directions as $dir)
                        <option value="{{ $dir->value }}">{{ $dir->getLabel() }}</option>
                    @endforeach
                </select>
            </div>
            <div class="sm:w-40">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">{{ __('Direction') }}</th>
                        ...
                        <tr wire:key="declaration-{{ $declaration->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    {{ $declaration->direction->value === 'arrival' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400' : 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' }}">
                                    {{ $declaration->direction->getLabel() }}
                                </span>
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 dark:text-gray-400">{{ __('No declarations found') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
