        ...
        return Action::make('export_iform_xml')
            ...
            ->action(function (array $data, IntrastatService $service): BinaryFileResponse {
                ...
                return response()->download(
                    $zipFilename,
                    sprintf('intrastat_iform_arrival_%d_%02d_%s.zip', $date->year, $date->month, now()->format('His')),
                    ['Content-Type' => 'application/zip']
                )->deleteFileAfterSend(true);
            })
            ->modalHeading('iFORM XML Export - Beérkezések (OSAP 2012)')
            ->modalDescription('Válaszd ki az időszakot, majd exportáld a READY státuszú beérkezési nyilatkozatokat ZIP-be csomagolt iFORM XML formátumba. Ez a formátum közvetlenül feltölthető a KSH-Elektra rendszerbe (https://elektra.ksh.hu).')
